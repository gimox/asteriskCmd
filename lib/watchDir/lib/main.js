module.exports = function (app, config) {
    ...
    function sendStatus(obj, source) {
        ...
        if (obj.url && obj.uid && obj.Status && obj.url != 'undefined' && obj.uid != 'undefined') {

            request.post({url: obj.url, form: obj}, function (err, httpResponse, body) {

                logger.debug('Sending data: file:' + source + ' - Url: ' + obj.url + ' - uid: ' + obj.uid + ' - Status: ' + obj.Status);

                if (err) {
                    logger.debug('Sending error');
                    logger.error('error sending file:  file:' + source + ' Url: ' + obj.url + ' - id: ' + obj.uid + ' - Status: ' + obj.Status);
                    logger.error(err);
                    return;
                }
            ...
            logger.error('file: ' + source + ' - SKIPPED -  has not right params, can not be sent Url: ' + obj.url + ' - id: ' + obj.uid + ' - Status: ' + obj.Status);
        }
