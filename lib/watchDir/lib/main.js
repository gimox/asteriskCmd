module.exports = function (app) {
    ...
    function checkOutgoingdone() {
        ...
        /*
        watch.watchTree(outgoingdoneDir,{ignoreDotFiles:true}, function (f, curr, prev) {
            if (typeof f == "object" && prev === null && curr === null) {
                // Finished walking the tree
            } else if (prev === null) {
                logger.info('file is new');
            } else if (curr.nlink === 0) {
                logger.info('file is removed');
            } else {
                logger.info('file is cahnged');
            }
        });
        */
        ...
        watch.createMonitor(outgoingdoneDir,{ignoreDotFiles:true}, function (monitor) {
          ...
          //  monitor.files['/home/mikeal/.zshrc'] // Stat object for my zshrc.
            monitor.on("created", function (f, stat) {
                logger.info('file is new '+f);
            });
            monitor.on("changed", function (f, curr, prev) {
                logger.info('file '+f+' is changed');
            });
            monitor.on("removed", function (f, stat) {
                logger.info('file '+f+' is removed');
            });
